  ...
  return (
    isLoading ? <Spinner /> : (
      <section className='flex flex-col py-28 items-center'>
        ...
        <p className='text-fs-md mt-4'>Have an account? <Link to='/login' className='text-primary'>Log In</Link></p>
        <form onSubmit={onSubmit} className='my-6 space-y-7'>
          <div className='flex flex-col space-y-8'>
            <input className='min-h-12 max-w-full rounded-md p-3 drop-shadow-sm' type="text" name="fullName" id="fullName" placeholder='Enter your full Name' value={fullName} onChange={onChange} />
            ...
            <input className='min-h-12 max-w-full rounded-md p-3 drop-shadow-sm' type="password" name="password2" id="password2" placeholder='Confirm Password' value={password2} onChange={onChange} />
          </div>


          <Button className="min-w-full" intent="primary" type='submit'> Sign up</Button>
